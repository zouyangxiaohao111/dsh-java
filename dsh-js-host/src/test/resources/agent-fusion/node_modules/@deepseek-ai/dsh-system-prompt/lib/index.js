 ...
 *
 * Overlay: real source (packages/core/system-prompt/src/index.ts) type-stripped
 * (erasable-only). `@deepseek-ai/dsh-llm` is type-only and stripped. The value
 * imports resolve to the overlays/shim: `{ Context, Service }` → the
 * @deepseek-ai/cordis shim (Service bridges registration into the Java core),
 * `z` → the schemastery overlay, and the dsh-scope primitives → the dsh-scope
 * overlay.
 *
